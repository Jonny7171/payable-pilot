...
August 30 update: I added a live supplier check through SerpApi. The supplier in the fixture is fictional, so the search returned unverified instead of inventing a risk claim. The public evidence file records the six-tool Strands run, both SerpApi search IDs, the $200 calculation, and the human review state.
