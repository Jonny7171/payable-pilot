...
let frameDirectory = root.appendingPathComponent("output/devpost/video-frames")
let outputURL = root.appendingPathComponent("output/devpost/payable-pilot-demo.mp4")
let frameRate: Int32 = 30
let width = 1920
let height = 1080
let durations = [10, 18, 16, 18, 16, 12]
...
func makePixelBuffer(image: CGImage, progress: Double, animate: Bool) -> CVPixelBuffer {
    ...
    let zoom = animate ? 1.0 + (0.015 * progress) : 1.0
...
for sceneIndex in 1...6 {
    ...
    for sceneFrame in 0..<sceneFrames {
        ...
        let buffer = makePixelBuffer(image: image, progress: progress, animate: sceneIndex > 1 && sceneIndex < 6)
